...
from onshape_client.oas import BTMParameterQueryList148, BTMIndividualQuery138, BTCurveGeometryCircle115, \
    BTMSketchCurve4, BTMSketch151, BTFeatureDefinitionCall1406, BTDocumentParams, BTFeatureScriptEvalCall2377, BTMParameterEnum145, BTMParameterQuantity147, BTMFeature134
...
result_feature = client.part_studios_api.add_part_studio_feature(did=part_studio.did, wvm=part_studio.wvm, wvmid=part_studio.wvmid, eid=part_studio.eid, bt_feature_definition_call1406_bt_feature_definition_call1406=feature_definition)
...
deterministic_ids_response = client.part_studios_api.eval_feature_script(did=part_studio.did, wvm=part_studio.wvm, wvmid=part_studio.wvmid, eid=part_studio.eid, bt_feature_script_eval_call2377_bt_feature_script_eval_call2377=BTFeatureScriptEvalCall2377(script=get_sketch_face_fs), _preload_content=False, _check_input_type=False)
det_id = json.loads(deterministic_ids_response.data)["result"]["message"]["value"]

# Generate an extrude feature
tool_body_type = BTMParameterEnum145(value="SOLID", enum_name="ToolBodyType", parameter_id="bodyType")
operation_type = BTMParameterEnum145(value="NEW", enum_name="NewBodyOperationType",
                                     parameter_id="operationType")
region_query = BTMParameterQueryList148(parameter_id="entities",
                                      queries=[BTMIndividualQuery138(deterministic_ids=[det_id])])
length = BTMParameterQuantity147(expression="3.03*in", parameter_id="depth")
extrude_feature = BTMFeature134(bt_type="BTMFeature-134", name="My extrude", feature_type="extrude",
...
result_feature_extrude = client.part_studios_api.add_part_studio_feature(did=part_studio.did, wvm=part_studio.wvm, wvmid=part_studio.wvmid,
                                                eid=part_studio.eid,
