...
def test_get_features(cube, client):
    result = client.part_studios_api.get_part_studio_features(cube.did, cube.wvm, cube.wvmid, cube.eid, _preload_content=False)
...
def test_insert_line_sketch(client, part_studio):
    ...
    point = BTMSketchPoint158(y=-0.071735, x=-0.0564367610245, entity_id=POINT_ID)
    ...
    feature_definition = BTFeatureDefinitionCall1406(feature=sketch)
    client.part_studios_api.add_part_studio_feature(did=part_studio.did, wvm=part_studio.wvm, wvmid=part_studio.wvmid, eid=part_studio.eid, bt_feature_definition_call1406=feature_definition)
