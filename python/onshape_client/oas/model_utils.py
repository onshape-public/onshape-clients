...
class OpenApiModel(object):
    ...
    def set_attribute(self, name, value):
        ...
        if name in openapi_types:
            ...
            raise ApiKeyError(
                "{0} has no key '{1}'".format(type(self).__name__, name), path_to_item
            )
...
class ModelSimple(OpenApiModel):
    ...
    def __getattr__(self, name):
        ...
        path_to_item.append(name)
        raise ApiKeyError(
            "{0} has no key '{1}'".format(type(self).__name__, name), [name]
        )
...
def get_simple_class(input_value):
    ...
    if isinstance(input_value, type):
        ...
        six.PY2
        and isinstance(input_value, (str_py2, unicode_py2, str))
...
def check_validations(validations, input_variable_path, input_values):
    ...
    if (
        "max_length" in current_validations
    ...
    if (
        "min_length" in current_validations
    ...
    if (
        "max_items" in current_validations
    ...
    if (
        "min_items" in current_validations
    ...
    if (
        "exclusive_maximum" in current_validations
    ...
    if (
        "inclusive_maximum" in current_validations
    ...
    if (
        "exclusive_minimum" in current_validations
    ...
    if (
        "inclusive_minimum" in current_validations
    ...
    if "regex" in current_validations and not re.search(
        ...
        raise ApiValueError(
            ...
            r"`%s` with flags=`%s`"
            % (input_variable_path[0], current_validations["regex"]["pattern"], flags)
        )
...
def remove_uncoercible(
    required_types_classes, current_item, from_server, must_convert=True
):
...
def change_keys_js_to_python(input_dict, model_class):
    ...
    output_dict = {}
    reversed_attr_map = {
        value: key for key, value in six.iteritems(model_class.attribute_map)
    }
...
def deserialize_model(
    model_data, model_class, path_to_item, check_type, configuration, from_server
):
...
def recursive_discriminator(model_class):
    ...
    for discriminator_key, discriminator_dict in model_class.discriminator().items():
        for child_class in discriminator_dict.values():
            for (
                child_discriminator_key,
                child_discriminator_dict,
            ) in recursive_discriminator(child_class).items():
                if child_discriminator_key in recursive_discriminated_types:
                    recursive_discriminated_types[child_discriminator_key].update(
                        child_discriminator_dict
                    )
                else:
                    recursive_discriminated_types[
                        child_discriminator_key
                    ] = child_discriminator_dict
...
def validate_and_convert_types(
    ...
    if not valid_type:
        ...
        flattened_subclasses = [
            subclass for subclass_list in subclasses for subclass in subclass_list
        ]
